...
const SortableHeader: React.FC<SortableHeaderProps> = ({
    ...
    const handleResizeStart = (e: React.PointerEvent) => {
        // Crucial: Stop propagation to prevent useSortable from capturing the pointer event
        e.preventDefault();
        e.stopPropagation();
    ...
    return (
        <th
            ref={setNodeRef}
            style={style}
            className={`py-2.5 ${column.width && column.width < 60 ? 'px-1' : 'px-3'} text-left border-b border-gray-200 text-xs font-medium text-gray-500 border-r border-gray-100 group bg-white transition-colors touch-none ${isDragging ? 'cursor-grabbing' : 'cursor-pointer hover:bg-gray-50'}`}
            {...attributes}
            {...listeners}
            onClick={handleHeaderClick}
        >
            <div className={`flex items-center ${column.width && column.width < 60 ? 'justify-center w-full' : 'gap-1.5'} relative h-full`}>
            ...
            {activeHeaderMenu === column.id && (
                ...
                >
                    ...
                    <div className="px-2 py-1 space-y-0.5">
                        ...
                        {onDeleteAttribute && !isProtectedColumn(column.accessorKey) && !isProtectedColumn(column.id) && (
                            <div
            ...
            {/* Resize Handle */}
            <div
                className="absolute right-0 top-0 bottom-0 w-2 -mr-1 cursor-col-resize group-hover:bg-blue-400/30 transition-colors z-20"
